...
{
    public class PositionEditor : TypeSelectorEditor
    {
        public PositionEditor() 
            : base(new[] {typeof(StaticPositionInitializer), typeof(RandomRegionPositionInitializer)})
        {
        }
        ...
        protected override void UpdateSelectedTypeFromSettings()
        {
            SelectedType = (EmitterSettings.Initializers ?? Array.Empty<IParticleInitializer>())
                .Where(x => x != null)
                .Where(x => x.InitializerType == InitializerType.Position)
                .Select(x => x.GetType())
                .FirstOrDefault();
        }
        ...
        protected override void NewTypeSelected()
        {
            ...
            {
                CommandHandler.Execute(new UpdateInitializerCommand(InitializerType.Position, null));
            }
            else
            {
