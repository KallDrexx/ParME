...
{
    public class VelocityEditor : TypeSelectorEditor
    {
        public VelocityEditor() 
            : base(new [] {typeof(RadialVelocityInitializer), typeof(RandomRangeVelocityInitializer)})
        {
        }
        ...
        protected override void UpdateSelectedTypeFromSettings()
        {
            SelectedType = (EmitterSettings.Initializers ?? Array.Empty<IParticleInitializer>())
                .Where(x => x != null)
                .Where(x => x.InitializerType == InitializerType.Velocity)
                .Select(x => x.GetType())
                .FirstOrDefault();
        }
        ...
        protected override void NewTypeSelected()
        {
            ...
            {
                CommandHandler.Execute(new UpdateInitializerCommand(InitializerType.Velocity, null));
            }
            else
            {
